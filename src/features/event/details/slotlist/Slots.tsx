		...
		{slots.map((slot) => {
			...
			return (
				<Box key={slot.id} className={classes.slotWrapper}>
					<Text className={cx(classes.slotNumber, ownSlotClass, ownSlot ? classes.ownSlotMarker : undefined)}
						  mod={{slot_item: true}}>
						{slot.number}
					</Text>
					<Group className={ownSlotClass} mod={{slot_item: true}} wrap={'nowrap'} gap={3} align={'start'}>
						...
						<Required itemBefore={!!slot.reservedFor} requirementList={slot.requirements}/>
					</Group>
